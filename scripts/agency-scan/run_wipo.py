#!/usr/bin/env python3
"""WIPO Taleo scraper using Playwright.

Portal: wipo.taleo.net/careersection/wp_2_pd/jobsearch.ftl
"""
...
HARD_REJECT = re.compile(
    ...
    r"analyst|clerk|assistant|patent|trademark|copyright|ip|"
    r"intellectual property|legal officer|translation|linguist|conference|"
    r"meeting)", re.I)
...
def main():
    print(f"WIPO Taleo scraper — {datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    with sync_playwright() as p:
        ...
        page.goto("https://wipo.taleo.net/careersection/wp_2_pd/jobsearch.ftl", wait_until="domcontentloaded")
        ...
        for job in ict_jobs:
            title = job['title']
            href = job['href']
            job_id_m = re.search(r'[?&]job=(\d+)', href)
            job_id = job_id_m.group(1) if job_id_m else sanitize(title)[:20]
            
            out = DIR / f"WIPO_{job_id}_{sanitize(title)[:50]}.md"
    ...
    total = len(list(DIR.glob("WIPO_*.md")))
