#!/usr/bin/env python3
"""IFAD PeopleSoft scraper using Playwright.

Portal: job.ifad.org (PeopleSoft)
"""
...
def main():
    print(f"IFAD PeopleSoft scraper — {datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    with sync_playwright() as p:
        ...
        () => {
            const seen = new Set();
            const results = [];
            // PeopleSoft uses table rows with links
            document.querySelectorAll('a[href*="HRS_APP_SCHJOB"], a[href*="HRS_CG_SEARCH"], .PSHYPERLINK').forEach(a => {
                const href = a.href || '';
            ...
            // Also try regular job links
            document.querySelectorAll('a[href*="job"], a[href*="vacanc"]').forEach(a => {
                const href = a.href || '';
                const text = a.innerText.trim();
                if (href && text.length > 5 && !seen.has(href)) {
                    seen.add(href);
        ...
        for job in ict_jobs:
            title = job['title']
            href = job['href']
            job_id_m = re.search(r'jobId[=:](\d+)', href) or re.search(r'/(\d{5,})/', href)
            job_id = job_id_m.group(1) if job_id_m else sanitize(title)[:20]
            
            out = DIR / f"IFAD_{job_id}_{sanitize(title)[:50]}.md"
    ...
    total = len(list(DIR.glob("IFAD_*.md")))
